...
const TranscribeAndSummarizeFromSlackStrategy = {
  ...
  async execute({ channelId, threadTs, commandAction, commandContext }) {
    ...
    try {
      ...
      // 5. 要約（AIサービス呼び出し、commandContextをプロンプトに反映）
      const summaryPrompt = buildSummaryPrompt(transcript, commandContext);
      const summary = await aiService.processMediaFile({
      ...
      logger.error(`TranscriptionAndSummaryFromSlackStrategy: エラー発生: ${error.message}`, { error });
      await slackService.postMessage({
        channel: channelId,
